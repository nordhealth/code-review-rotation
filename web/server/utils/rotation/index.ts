import { runDevRotation, type RotationDeveloper } from "./developers";
import { runTeamRotation, type TeamRotationContext, type RotationSquad } from "./teams";
import {
  teamDevelopers,
  developers,
  preferableReviewers,
...
async function loadPreviousDevAssignments(
  ...
  if (assigns.length === 0) return result;
...
export async function executeDevRotation(
  ...
  return [...assignments.entries()].map(([devId, reviewerIds]) => ({
    targetId: devId,
    reviewerIds,
  }));
}
...
export async function executeTeamRotation(
  ...
  return [...assignments.entries()].map(([squadId, reviewerIds]) => ({
    targetId: squadId,
    reviewerIds,
  }));
}

...
export function selectMostAvailable(
  ...
  shuffled.sort((a, b) => {
    ...
    if (aWasPrevious !== bWasPrevious)
      return aWasPrevious - bWasPrevious
...
export function runAllocationAlgorithm(
  ...
  for (const dev of devs) {
    ...
    const assignedExp = new Set([...dev.assignedReviewerIds].filter(id => experiencedIds.has(id)))
    const _assignedUnexp = new Set([...dev.assignedReviewerIds].filter(id => validUnexpIds.has(id)))
    ...
    if (!isExperienced && currentUnexpReviewers.length > 0) {
      for (const unexpId of currentUnexpReviewers) {
        ...
        /* v8 ignore next 2 */
        if (!dev.assignedReviewerIds.has(unexpId))
          continue
    ...
    if (isExperienced && currentUnexpReviewers.length > 1) {
      ...
      const _toKeep = currentUnexpReviewers[0]
  ...
  for (const unexpDev of unassignedUnexp) {
    ...
    for (const expDev of devs) {
      if (!experiencedIds.has(expDev.id))
        continue
      /* v8 ignore next 2 */
      if (expDev.id === unexpDev.id)
        continue

import { teams, teamDevelopers } from '../../db/schema'
...
export async function createTeam(data: { name: string; defaultReviewerCount?: number }) {
  const slug = slugify(data.name)
  const [team] = await db.insert(teams).values({ ...data, slug }).returning()
  return team
}

export async function updateTeam(
  id: string,
  data: Partial<{ name: string; defaultReviewerCount: number }>,
) {
